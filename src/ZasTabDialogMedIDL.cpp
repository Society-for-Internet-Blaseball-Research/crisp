...
extern ZasTabDialogMedIDL *_ztdmIDL;

extern int DefaultStock;

ZasTabDialogMedIDL::ZasTabDialogMedIDL(idcIDL *idc, ZatTabDialog *dp)  : ZatTabDialogMed(idc,dp), df_(DefaultStock), idcIDL_(idc) {
...
ZasTabDialogMedIDL::~ZasTabDialogMedIDL() {
	_ztdmIDL = NULL;
}

void ZasTabDialogMedIDL::cbEditSliderDoubleModified(const double delta, const int id){
	if(!lock_){	
		lock_=1;
		if(!dp_->isBoxChecked(id,0)){
			idcIDL_->ModifyCache(id+(dp_->numItemsPerTab()-1)*dp_->currentIndex(), dp_->isBoxChecked(id,1), delta, dp_->selectComboBox(),delta_);
		}
		else{
			for(size_t i=0;i<dp_->numItems();i++)
				if(dp_->ZatDialogBase::isBoxChecked(i,0))
					idcIDL_->ModifyCache(i, dp_->ZatDialogBase::isBoxChecked(i,1), delta, dp_->selectComboBox(),delta_);
			listUpdate();
		}
		lock_=0;
	}
...
void ZasTabDialogMedIDL::addListItems(){
	if(!lock_){
		lock_=1;
	    dp_->clearComboBox();
...
void ZasTabDialogMedIDL::listUpdate(){
	...
	if(!(dat = idcIDL_->getDat(dp_->selectComboBox())))
		return;
