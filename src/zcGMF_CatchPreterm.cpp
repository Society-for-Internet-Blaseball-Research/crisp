...
zcGMF_CatchPreterm::zcGMF_CatchPreterm(ZatGraphDialog *gd) : zcGM_YearGraph(gd) {
	drawLabels();
}

void zcGMF_CatchPreterm::drawLabels(void){
...
void zcGMF_CatchPreterm::setTitle(int index){
	if(index < Fisheries.num()){
	    char title[80];
		DefaultSelection = index;
		sprintf(title, "Preterminal Catch, %s Fishery", ::Fisheries[DefaultSelection].get_name());
		_gd->setTitleLabel(title);
	}
}

void zcGMF_CatchPreterm::getData(void){
	...
	for (Age age = FIRST_HV_AGE; age <= age.top(); age++){
		for (int i = 0; i < ny; i++){
			tempVal=::Fisheries[DefaultSelection].get_preterm_catch(age, i);
			if(tempVal>maxVal)
				maxVal = tempVal;
	...
	for (int i = 0; i < ny; i++){
		tempVal=::Fisheries[DefaultSelection].get_preterm_catch(i);
		if(tempVal>maxVal)
			maxVal = tempVal;
		dataList.insert(tempVal);
	}
